...
!
! $Id: vectorUtilities.f90 9212 2019-02-04 13:55:12Z spee $
!
!>
...
module vectorUtilities
    ...
    public :: trapeziumRule, logLinearInterpolateInv, intgrateWithExponentComponent
...
!!   @ingroup general
function interpolateLine(x1, x2, f1, f2, x, ierr, errorMessage)
    ...
    if (abs(dx) > eps) then
        ...
        if (abs(dx2) > eps) then
            ierr = -1
            errorMessage = 'division by zero in interpolateLine'
        else
...
!!   @ingroup general
function linearInterpolate( X, Y, xx, ierr, errorMessage )
    ...
    if ( nelem == 1 ) then
        ...
        ierr = 1
        errorMessage = "linearInterpolation: no data points"
    endif
    !
    ...
    if ( size(x) /= size(y) ) then
        ierr = 1
        errorMessage = "linearInterpolation: x- and y-vectors should have the same length!"
    endif
    ...
    if (ierr == 0) then
        ...
        linearInterpolate = interpolateLine( x(idxm1), x(idx), Y(idxm1), Y(idx), xx, ierr, errorMessage )
...
!> Function for linearly interpolating and extrapolating
!!   @ingroup general
function linearInterpolateMulti( X, Y, xx, ierr, errorMessage, nUnique )
    ...
    if (ierr == 0) then
        ...
        do i = 1,size(Y,dim=2)
            linearInterpolateMulti(i) = interpolateLine( x(idxm1), x(idx), Y(idxm1,i), Y(idx,i), xx, ierr, errorMessage )
        end do
...
!> Function for linearly interpolating and extrapolating
!!   @ingroup general
function linearInterpolateMulti2D( X1, X2, Y, xx1, xx2, ierr, errorMessage )
    ...
    if (ierr /= 0) then
        write(errorMessage,*) 'Allocate error in linearInterpolateMulti2D with size ', size(Y,dim=2), ' x ', size(Y,dim=3)
        return
    endif
    ...
    if ( nelem1 == 1 ) then
        ...
        do j = 1, nelem2
            y2(j,:)=linearInterpolateMulti( X1, Y(:,j,:), xx1, ierr, errorMessage )
        end do
    endif
    
    !
    if (ierr == 0) then
        linearInterpolateMulti2D = linearInterpolateMulti( X2, Y2, xx2, ierr, errorMessage )
    endif
...
!!   @ingroup general
function logLinearInterpolate( X, Y, xx, ierr, errorMessage )
...
!
    ...
    if ( size(x) /= size(y) ) then
        ierr = 1
        errorMessage = "loglinearInterpolation: x- and y-vectors should have the same length!"
    endif
    !
    ! Take care of a special case: only one data point
    !
    ...
    if ( any(x < 0 ) ) then
        ierr = 1
        errorMessage = "The vector X in function loglinearInterpolate contains non-positive values!"
    endif
    !
    ! Also check that xx is positive
    if (xx <=0) then
        ierr = 1
        errorMessage = "Argument xx in function loglinearInterpolate has non-positive value!"
    endif
    !
    !
    if (ierr == 0) then
        ...
        if (abs(x1 - x2) < eps) then
            if (abs(xx - x1) < tol) then
                ...
                ierr = -1
                errorMessage = 'equal x values found in logLinearInterpolate'
            endif
        else if (x1 > 0.0_wp .and. x2 > 0.0_wp) then
            logLinearInterpolate = interpolateLine( log(x1), log(x2), Y(idxm1), Y(idx), log(xx), ierr, errorMessage )
        else
            !
            ! fall back on normal interpolation
            !
            logLinearInterpolate = interpolateLine( x1, x2, Y(idxm1), Y(idx), xx, ierr, errorMessage )
        endif        
...
!!   @ingroup general
function logLinearInterpolateInv( X, Y, xx, ierr, errorMessage )
...
!
    ...
    if (ierr == 0) then
        ...
        if ( Y(idxm1) <= 0 .or. Y(idx) <= 0 ) then
            ierr = 1
            errorMessage = "The vector Y in function loglinearInterpolate contains non-positive values!"
        else
            logLinearInterpolateInv = exp(interpolateLine( x(idxm1), x(idx), log(Y(idxm1)), log(Y(idx)), xx, ierr, errorMessage ))
        endif
...
!!   @ingroup general
function linearInterpolateCyclic( X, Y, xx, ierr, errorMessage )
    ...
    if ( size(x) /= size(y) ) then
        ierr = 1
        errorMessage = "linearInterpolationCyclic: x- and y-vectors should have the same length!"
    endif
    ...
    if (ierr == 0) then        
        ...
        !
        linearInterpolateCyclic = interpolateLine( Xsorted(idxm1), Xsorted(idx), Y(IDXsorted(idxm1)), Y(IDXsorted(idx)), xx, ierr, errorMessage )
...
!!   @ingroup general
function logLinearInterpolateCyclic( X, Y, xx, ierr, errorMessage )
    ...
    if ( size(x) /= size(y) ) then
        ierr = 1
        errorMessage = "loglinearInterpolationCyclic: x- and y-vectors should have the same length!"
    endif
    !
    ! This is log-interpolation: check that elements of x have positive values
    if ( any(x < 0) ) then
        ierr = 1
        errorMessage = "The vector X in function loglinearInterpolateCyclic contains non-positive values!"
    endif
    !
    ! Also check that xx is positive
    if (xx <=0) then
        ierr = 1
        errorMessage = "Argument xx in function loglinearInterpolateCyclic has non-positive value!"
    endif
    !
    !
    ...
    if (ierr == 0) then
        ...
        if (x1 > 0.0_wp .and. x2 > 0.0_wp) then
            logLinearInterpolateCyclic = interpolateLine( log(x1), log(x2), Y(IDXsorted(idxm1)), Y(IDXsorted(idx)), log(xx), ierr, errorMessage )
        else
            logLinearInterpolateCyclic = interpolateLine( x1, x2, Y(IDXsorted(idxm1)), Y(IDXsorted(idx)), xx, ierr, errorMessage )
        endif
...
!!   @ingroup general
function linearInterpolateAngles( X, Y, xx, ierr, errorMessage, sinY )
    ...
    if ( nelem == 1 ) then
        ...
        ierr = 1
        errorMessage = "linearInterpolateAngles: no data points"
    endif
    !
    ! Check that X and Y are the same size
    !
    if ( size(x) /= size(y) ) then
        ierr = 1
        errorMessage = "linearInterpolateAngles: x- and y-vectors should have the same length!"
    endif
    ...
    if (ierr == 0) then
        ...
        if (present (sinY)) then
                           cosAngle = interpolateLine( x(idxm1), x(idx),    Y(idxm1),    Y(idx), xx, ierr, errorMessage ) ! Y is actually sin(Y*pi/180)
            if (ierr == 0) sinAngle = interpolateLine( x(idxm1), x(idx), sinY(idxm1), sinY(idx), xx, ierr, errorMessage )
        else
                           cosAngle = interpolateLine( x(idxm1), x(idx), cos(Y(idxm1) * pi / 180.0D0), cos(Y(idx) * pi /180.0D0), xx, ierr, errorMessage )
            if (ierr == 0) sinAngle = interpolateLine( x(idxm1), x(idx), sin(Y(idxm1) * pi / 180.0D0), sin(Y(idx) * pi /180.0D0), xx, ierr, errorMessage )
        end if
...
!!   @ingroup general
function stepSize( i, x, ierr, errorMessage )
    ...
    if ((i < 1) .or. (i > nx)) then
        ...
        !
        ierr = 1
        write(errorMessage, '(a,i0)') "The index i in the function stepSize isn't an element of the vector x", i
...
!> Function for the Trapezium rule
!!   @ingroup general
function trapeziumRule( x, f, ierr, errorMessage )
    ...
    if (size(x) /= size(f)) then
        ...
        !
        ierr = 1
        write(errorMessage, '(a,i0)') "The two vector in the function trapezium rule aren't of the same size", nx
    endif
    !
    ...
    do ix = 1, nx
        if (ierr == 0) then
            trapeziumRule = trapeziumRule + f(ix) * stepSize (ix, x, ierr, errorMessage)
        endif
    enddo

end function trapeziumRule

!> Function for the Trapezium rule with exponent component
!!   @ingroup general
function intgrateWithExponentComponent( x, f, ierr, errorMessage  )

    real (kind=wp),   intent(in)    :: x(:)                            !< vector of x-values
    real (kind=wp),   intent(in)    :: f(:)                            !< vector of function-values
    integer,          intent(out)   :: ierr                            !< error code, 0: success
    character(len=*), intent(inout) :: errorMessage                    !< error message, only set in case of an error
    real (kind=wp)                  :: intgrateWithExponentComponent   !< value after the Trapezium Rule
    ...
    if (size(x) /= size(f)) then
        ...
        !
        ierr = 1
        write(errorMessage, '(a,i0)') "The two vector in the function intgrateWithExponentComponent aren't of the same size", nx
        call set_nan(intgrateWithExponentComponent)
    else
        !
        ! Compute the integral by using the Trapezium rule
        !
        nx = size(x,1)
        intgrateWithExponentComponent = 0.0
        do ix = 1, nx-1
            intgrateWithExponentComponent = intgrateWithExponentComponent + min( 0.5d0 * ( exp(-x(ix+1)) * f(ix+1) +  exp(-x(ix)) * f(ix) ) * ( x(ix+1) - x(ix) ), &
                                                                                 exp(-x(ix)) - exp(-x(ix+1) )  )
        enddo
    endif

end function intgrateWithExponentComponent
