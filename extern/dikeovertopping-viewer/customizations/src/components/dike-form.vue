      ...
      <tbody>
        ...
        >
          <template v-if="points[index + 1]">
            ...
            <td class="dike-form__field dike-form__field--static">
              1 : {{ slopeRatio(row, points[index + 1]) }}
            </td>
            <td class="dike-form__field">
              <button
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      slopeRatio(points, nextPoints) {
        const slope =
          1 / ((points.y - nextPoints.y) / (points.x - nextPoints.x));
        return slope === -Infinity || isNaN(slope) ? '∞' : slope.toFixed(0);
      },
    }
  };
</script>
